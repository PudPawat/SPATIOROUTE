...
Specialized for Qwen3-VL models with optimized defaults
...
def check_qwen3_model(model_name: str) -> bool:
    """Check if model name is a Qwen3-VL model"""
    model_name_lower = model_name.lower()
    return "qwen3" in model_name_lower or ("3." in model_name and "vl" in model_name_lower)
...
def load_checkpoint(checkpoint_file: str) -> Tuple[Dict, Set[Any], Set[Any]]:
    """
    Load checkpoint file to resume evaluation
    
    Args:
        checkpoint_file: Path to checkpoint JSON file
        
    Returns:
        Tuple of (checkpoint_data_dict, processed_question_ids_set, oom_error_ids_set)
        - processed_ids: IDs of successfully processed samples (to skip)
        - oom_error_ids: IDs of OOM error samples (to rerun)
    """
    ...
    # Separate OOM errors from successfully processed samples
    processed_ids = set()
    oom_error_ids = set()
    
    for r in existing_results:
        question_id = normalize_question_id(r.get("question_id"))
        ...
        if (
            ...
            or "CUDA_OUT_OF_MEMORY" in str(r.get("predicted_answer", ""))
        ):
            oom_error_ids.add(question_id)
        else:
...
def evaluate_with_cot(
    ...
    With ``thinking_mode=True`` (Qwen3-VL-Thinking style), each pass calls
    ``answer_question_from_images_with_thinking``; pass 2 is fed the **full** pass-1 decode
    (thinking + visible tail) as ``REASONING FROM FIRST ANALYSIS``. Metrics use the pass-2
    **after-think** segment only (same as ``evaluate_sqa_qwen3.py --thinking``).
    ...
    Args:
        ...
        top_p: Nucleus top-p for generation
        thinking_mode: Use Qwen thinking split for both passes
        think_end_token_id: End-of-thinking token id (Qwen3-VL-Thinking default 151668)
    ...
    if resume_from and os.path.exists(resume_from):
        ...
        # Load existing results, but remove OOM errors (they will be rerun)
        ...
        # Load existing metrics and counts
    ...
    samples_total = len(samples) + total  # Total including already processed
...
Based on your previous reasoning about the spatial layout, positions, and directions, provide a direct and concise answer to the question:"""
            ...
            if output_file:
                ...
                try:
                    ...
                    print(f"⚠️  Warning: Failed to save checkpoint: {save_error}")
            ...
            if "out of memory" in error_msg.lower() or "cuda" in error_msg.lower():
                ...
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
                    gc.collect()
                print("Consider using --load-in-8bit (recommended for Qwen3-VL), or reducing image count")
            ...
            if "'parameter' object has no attribute 'cb'" in error_msg.lower() or "has no attribute 'CB'" in error_msg.lower():
                print("\n❌ Qwen3-VL vision encoder quantization error detected!")
                ...
                print("   Example: python evaluate_cot_qwen3.py --split test --input-type video --model-name Qwen/Qwen3-VL-2B-Instruct --no-quantization")
                print("\n   Note: This requires more GPU memory (~8-10 GB for 2B model)")
                print("   See QWEN3_FIXES.md for more details.")
    ...
    # Save final results (or checkpoint if interrupted)
    if output_file:
        save_checkpoint(
            output_file=output_file,
            results=results,
        ...
        # Add additional metadata to final checkpoint
        try:
            ...
            attach_result_json_metadata(final_data, evaluation_settings)
            with open(output_file, 'w') as f:
                json.dump(final_data, f, indent=2)
        except Exception as e:
            print(f"⚠️  Warning: Failed to add metadata to checkpoint: {e}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Chain of Thought (CoT) Evaluation on SQA dataset (Qwen3-VL optimized)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Qwen3-VL-2B with video input (8-bit quantization, recommended)
  python evaluate_cot_qwen3.py --split test --input-type video --model-name Qwen/Qwen3-VL-2B-Instruct
  
  # Qwen3-VL-4B with render input
  python evaluate_cot_qwen3.py --split test --input-type render --model-name Qwen/Qwen3-VL-4B-Instruct
  
  # Without quantization (requires more GPU memory)
  python evaluate_cot_qwen3.py --split test --input-type video --model-name Qwen/Qwen3-VL-2B-Instruct --no-quantization

  # Qwen3-VL-Thinking: two-pass CoT with </think> split on both passes (higher max-new-tokens by default)
  python evaluate_cot_qwen3.py --split test --input-type video --thinking \\
    --model-name Qwen/Qwen3-VL-8B-Thinking --no-quantization

Note: Qwen3-VL models may have quantization compatibility issues similar to Qwen2.5-VL.
    ...
    parser.add_argument(
        ...
        help='Qwen3-VL model name or path (default: Qwen/Qwen3-VL-2B-Instruct)'
    )
    parser.add_argument(
        '--input-type',
        type=str,
    ...
    parser.add_argument(
        '--load-in-4bit',
        action='store_true',
        help='Load model in 4-bit quantization (⚠️ NOT RECOMMENDED for Qwen3-VL - may cause AssertionError)'
    )
    parser.add_argument(
        '--load-in-8bit',
        action='store_true',
        help='Load model in 8-bit quantization (RECOMMENDED for Qwen3-VL, default if no quantization flags specified)'
    )
    parser.add_argument(
        '--no-quantization',
        action='store_true',
        help='Disable quantization entirely (requires more GPU memory, most reliable for Qwen3-VL)'
    )
    parser.add_argument(
        '--max-new-tokens',
        type=int,
    ...
    parser.add_argument(
        '--thinking',
        action='store_true',
        help='Qwen3-VL-Thinking: split </think> reasoning vs answer on both CoT passes (see VideoLM.answer_question_from_images_with_thinking)'
    )
    parser.add_argument(
        '--temperature',
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help='Save checkpoint every N samples (default: 50). Set to 0 to disable periodic checkpoints.'
    )
    parser.add_argument(
        '--resume-from',
        type=str,
        default=None,
        help='Path to checkpoint file to resume from. If not specified, will auto-detect if output file exists and is a checkpoint.'
    )
    parser.add_argument(
        '--memory-threshold-gb',
        type=float,
        default=2.0,
        help='Minimum free GPU memory in GB before stopping to prevent OOM (default: 2.0 GB)'
    )
    ...
    # Validate model name
    if not check_qwen3_model(args.model_name):
        print(f"⚠️  Warning: Model '{args.model_name}' does not appear to be a Qwen3-VL model.")
        print("   This script is optimized for Qwen3-VL models.")
        print("   For Qwen2-VL models, use evaluate_sqa_cot_2pass_qwen2_vl.py instead.")
        print("   For Qwen2.5-VL models, use evaluate_cot_qwen25.py instead.")
    ...
    # Default: use 8-bit quantization (recommended for Qwen3-VL)
    ...
    load_in_4bit = False
    load_in_8bit = True  # Default to 8-bit for Qwen3-VL
    ...
    if args.no_quantization:
        ...
        print("   This is the most reliable option for Qwen3-VL if quantization fails")
    elif args.load_in_4bit:
        # User explicitly requested 4-bit (not recommended for Qwen3-VL)
        load_in_4bit = True
        load_in_8bit = False
        print("⚠️  WARNING: Using 4-bit quantization with Qwen3-VL may cause AssertionError!")
        print("   See QWEN3_FIXES.md for details.")
        print("   Recommended: Use --load-in-8bit instead (or omit flag, 8-bit is default)")
        response = input("   Continue with 4-bit anyway? (y/n): ")
        if response.lower() != 'y':
            print("Switching to 8-bit quantization (default for Qwen3-VL)...")
            load_in_4bit = False
            load_in_8bit = True
    ...
    if not output_file:
        ...
        if args.filter_scenes_json:
            # Use filter JSON name in filename
            filter_name = Path(args.filter_scenes_json).stem
            scenes_suffix = filter_name
        elif args.scenes:
            # Use scene IDs
            scenes_suffix = '_'.join(args.scenes)
            if len(scenes_suffix) > 100:  # Truncate if too long
                scenes_suffix = scenes_suffix[:100] + '...'
        else:
            scenes_suffix = 'all_scenes'
        think_suffix = '_thinking' if args.thinking else ''
        output_file = f'sqa_{args.split}_results_cot{think_suffix}_{args.input_type}_{model_suffix}_{scenes_suffix}.json'
    
    # Handle checkpoint resuming
    resume_from = None
    if args.resume_from:
        # Explicit resume file specified
        if not os.path.exists(args.resume_from):
            ...
            # Optionally use the same output file
            if not args.output:
                output_file = args.resume_from
    else:
        # Auto-resume: check if output file exists and has checkpoint
        if os.path.exists(output_file):
            try:
                ...
                if checkpoint_json_incomplete(checkpoint_data):
                    ...
                    # File exists but doesn't look like a checkpoint - ask or start fresh
                    print(f"\n⚠️  Output file exists: {output_file}")
                    print(f"   It appears to be a complete results file (not a checkpoint).")
                    print(f"   Starting fresh evaluation (will overwrite existing file).")
                    print(f"   To resume explicitly, use: --resume-from {output_file}")
                    resume_from = None
            except (json.JSONDecodeError, KeyError) as e:
                # File exists but can't be read as checkpoint
                print(f"\n⚠️  Output file exists but couldn't be read as checkpoint: {output_file}")
                print(f"   Starting fresh evaluation (will overwrite existing file).")
                resume_from = None
        else:
            resume_from = None
